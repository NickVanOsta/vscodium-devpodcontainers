// The code is from https://github.com/jeanp413/open-remote-ssh/blob/master/src/serverSetup.ts
// but with the following changes:
//   - ssh2 and simple-socket dependecies removed
//   - use spawnSync(ssh, ...) instead
// 
// It's likely that these changes broke Windows support.

import * as crypto from 'crypto';
import { spawnSync } from 'child_process';
import spawnShell, { isWindows } from "../shell";
...
export async function installCodeServer(devpodAddr: string, extensionIds: string[]): Promise<ServerInstallResult> {
    let platform =  undefined;
    const useSocketPath = false;
    const envVariables = ['SSH_AUTH_SOCK'];

    let shell = 'powershell';

    // detect plaform and shell for windows
    if (!platform || platform === 'windows') {
        const result = spawnSync(`ssh ${devpodAddr} -- uname -s`, { shell: spawnShell });
    ...
    if (platform === 'windows') {
        ...
        const out = spawnSync(command, { shell: spawnShell });
        commandOutput = {
        ...
        const out = spawnSync('ssh', [devpodAddr, '--', 'bash', '-c', `'${installServerScript.replace(/'/g, isWindows ? `''` : `'\\''`)}'`], { shell: spawnShell });
        commandOutput = {
            stdout: out.stdout.toString(),
            stderr: out.stderr.toString(),
        };
